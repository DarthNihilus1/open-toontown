...
class DistributedPartyTugOfWarActivityAI(DistributedPartyTeamActivityAI):
    ...
    def reportFallIn(self, losingTeam):
        self.notify.debug("reportFallIn( losingTeam=%s )" % PartyGlobals.TeamActivityTeams(losingTeam))
        ...
        if (self.losingTeam != PartyGlobals.TeamActivityNeitherTeam) and (losingTeam != self.losingTeam):
            self.notify.warning("Report of %s as the losingTeam doesn't match previously reported value. Ignoring." %PartyGlobals.TeamActivityTeams(losingTeam))
            return
