...
class EstateManagerAI(DistributedObjectAI.DistributedObjectAI):
    ...
    def __createEstateZoneAndObjects(self, avId, isOwner, ownerId, name):
        ...
        self.setEstateZone(avId, [self.zoneId, isOwner, name]) # [zoneId, isOwner, userName (if owner)]
        self.account2avId[name] = avId
        self.zone2owner[self.zoneId] = avId

        # start a ref count for this zone id
        self.refCount[self.zoneId] = []
        ...
        self.getEstate(avId, self.zoneId, callback)
    ...
    def __sendGetEstate(self, avId, context):
        ...
        See getEstate().
        """
        # first lets grab the account id from the avatar id using the fields in it
        # specifically the setDISLid field which is the account id
        # also use the context to keep track of the callback function to return to later
        self.context = context

        self.mgr.air.dbInterface.queryObject(self.mgr.air.dbId, avId, self._handleGetAccount)

    def _handleGetAccount(self, dclass, fields):
        """
        This is a callback from the database queryObject call in getEstate.
        It is used to get the account id from the avatar id.
        """
        # get the account id from the avatar id
        self.accountId = fields['setDISLid'][0]
        # now we can query the database for the estate
        self.mgr.air.dbInterface.queryObject(self.mgr.air.dbId, self.accountId, self.__handleGetEstateResp)
        
    def __handleGetEstateResp(self, dclass, fields):
        # Use the context to retrieve the callback parameter passed in
        # to getEstate().
        # grab estateId from  the account fields, ESTATE_ID is the field name
        self.estateId = fields.get('ESTATE_ID' , 0)
        self.avs = fields.get('ACCOUNT_AV_SET')
        ...
        if not self.estateId:
            ...
            self.__updateEstate()
        # now we can query the database for the estate
        self.mgr.air.dbInterface.queryObject(self.mgr.air.dbId, self.estateId, self.__handleGetEstateResp2, dclass, fields)

    def __updateEstate(self):
        """
        Updates the estate for the account.
        """
        # update the estate
        self.mgr.air.dbInterface.updateObject(self.mgr.air.dbId, self.accountId, self.dclassesByName['AccountAI'], {'ESTATE_ID' : self.estateId, })

    def __createEstate(self):
        """
        Creates a new estate for the account.
        """
        # create a new estate
        self.mgr.air.dbInterface.createObject(self.mgr.air.dbId, self.dclassesByName['DistributedEstateAI'], {}, self.__handleCreateEstateResp)

    def __handleCreateEstateResp(self, estateId):
        """
        This is a callback from the database createObject call in __createEstate.
        It is used to get the estate id from the database.
        """
        ...
        # now we can query the database for the estate
        self.__updateEstate()
        self.mgr.air.dbInterface.queryObject(self.dbId, self.estateId, self.__handleGetEstateResp2)

    def __handleGetEstateResp2(self, dclass, fields):
        ...
        for av in self.avs:
            name = self.mgr.air.doId2do[av].getName()
            index += 1
            if av != 0:
                houseId[i] = fields.get('setSlot%dToonId' % (index), 0)
                # there will only be a houseId if there is a toonId in the slot
                # for example only 2 toons in the estate will have houseId's
                # houseId = 0 means no toon in the slot
                if houseId[i] == 0:
                    # we need to make a house for this toon
                    self.mgr.air.dbInterface.createObject(self.mgr.air.dbId, self.dclassesByName['DistributedHouseAI'], {'setName': [name],
                                                'setAvatarId': [av],
                                                }, self.__handleCreateHouseResp, i)
                
            else:
                # we need to make an empty house
                self._makeEmptyHouse(index)            
            self.notify.debug(f"houseId = {houseId[i]}")
        
        # now go through each house and retrieve the info in the database 
        # also go through the creation process if needed along with making blank houses if needed 
        # and then call DistributedEstateAI's initEstateData func
        for i in range(numHouses):
            if houseId[i] != 0:
                self.mgr.air.dbInterface.queryObject(self.mgr.air.dbId, houseId[i], self.__handleGetHouseResp)
            else:
                
    def __handleGetHouseResp(self, dclass, fields, index):
        self.mgr.air.sendActivate(self.houseId, self.mgr.air.districtId, self.estate.zoneId, self.mgr.air.dclassesByName['DistributedHouseAI'], 
                                  {
                                    'setColor': [index],
                                    'setHousePos': [index],
                                    'setAvatarId': [self.avs[index]],
                                    'setName': [self.mgr.air.doId2do[self.avs[index]].getName()],
                                  }
                                  )
        # now wait for the DistributedHouseAI to be generated 
        self.acceptOnce(f'generate-{self.houseId}', self.__handleGenerateHouse, index)
    
    def __handleGenerateHouse(self, house, index):
        house.estate = self.estate
        house.interior.gender = self.mgr.air.doId2do[self.avs[index]].style.getGender()
        house.interior.start()

        self.house = house
        self.estate.houses[index] = self.house
        if config.GetBool('want-gardening', True):
            self.house.createGardenManager()
    def __handleCreateHouseResp(self, houseId, index):
        """
        This is a callback from the database createObject call.
        """
        # update the house id for the specific avatar

        av = self.mgr.air.doId2do[self.avs[index]]
        if not av:
            self.notify.warning(f'__handleCreateHouseResp: av {self.avs[index]} not present')
            return
        av.b_setHouseId(houseId)
        self.houseId = houseId
       
    def _makeEmptyHouse(self, index):
        """
        Creates an empty house for the account.
        
        
        """
        self.house = DistributedHouseAI.DistributedHouseAI(self.mgr.air)
        self.house.setColor(index)
        self.house.setHousePos(index)
        ...
        if (retCode == 0):
            ...
            # numHouseKeys is the number of keys for each house
            ...
            # numHouseVal is the number of values for each house
